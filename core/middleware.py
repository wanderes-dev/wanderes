...
# search-ranking signal separately from SITE_DOMAIN (see PROJECT_STATE.md
# for how each of these came to exist). Deliberately an explicit,
# hand-maintained list rather than "any host that isn't SITE_DOMAIN" -
# Render assigns the live service its own current onrender.com hostname
# (currently wanderes-web.onrender.com) and very likely uses that same
# hostname for its own healthCheckPath probe (render.yaml); blanket-
# redirecting every non-canonical host would risk redirecting that health
# check too, which Render would read as the service being down. Only add
# a new alias here once it's confirmed to be a public-facing duplicate,
# never Render's own current service hostname.
#
# www.wanderes.com is deliberately NOT in this list, even though it looks
# like the obvious "other" alias to consolidate. Something outside this
...
# that created an infinite apex->www->apex redirect loop and took the
# entire public site down (ERR_TOO_MANY_REDIRECTS) for several minutes
# before it got caught and reverted. SITE_DOMAIN is now www.wanderes.com
# to match that reality instead of fighting it - never re-add
# www.wanderes.com here without first confirming, live, that nothing
# upstream still redirects apex to www.
...
class CanonicalDomainRedirectMiddleware:
    ...
    host - a health check must never get a redirect instead of a 200, on
    any hostname, or Render would read the service as down."""
...
class UserLanguagePreferenceMiddleware:
    ...
    Accept-Language header - a previously saved preference should always
    be respected, regardless of which device/browser they're on right
    now. Must sit after AuthenticationMiddleware in settings.MIDDLEWARE
    (request.user isn't resolved yet before that) and after
    LocaleMiddleware (this deliberately overrides its result rather than
    racing it) - see the ordering comment in settings/base.py.
