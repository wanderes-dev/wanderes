...
**Status update (2026-09-11): CLOSED - Booking.com property-level data access confirmed unavailable, `integrations/hotels/booking_com.py` removed.** The founder emailed CJ's Booking.com program contact directly, asking whether any property-level data source at all - a product feed, a Travel & Experience feed, CJ Product Search, a hotel catalogue, anything - exists for CJ publishers to programmatically discover Booking.com properties, given Demand API access was already known to be out of reach. CJ's answer: **"Unfortunately, no property-level feed is currently available."** Demand API access is restricted to strategic partners outside Wanderes's publisher category; the relationship stays managed through CJ. What CJ does offer - deep links to specific pages, and search widgets - is exactly what `integrations/affiliates/CJAffiliateProvider` (§ above) already targets.

**This closes the question, not just leaves it pending.** `integrations/hotels/booking_com.py` and its registration in `get_hotel_provider()`'s provider registry have been removed - there was no approved path to build real property-level search against Booking.com, so keeping a skeleton "ahead of access" no longer made sense. `integrations/hotels/base.py`'s `HotelProvider`/`HotelOption` interface stays, provider-agnostic, for whichever accommodation provider gets evaluated next (Priceline Partner Solutions remains the one named-but-unresearched candidate - `DECISIONS_PENDING.md` §4). `integrations/affiliates/CJAffiliateProvider` is untouched - it was never Booking.com-Demand-API-dependent to begin with, and CJ's own guidance in this exchange (deep links + widgets) confirms it's the right tool for what's actually available.

### 13.9 Accommodation search links shipped via CJ Deep Link Automation (2026-09-23)

**Status update: no property-level search, but a real, shipped outbound feature.** §13.8 closed the question of Booking.com *inventory* access - it stays closed, nothing here reopens it. What changed is that CJ's **Deep Link Automation** was validated in production (a temporary page, `/test-cj-deeplink/`, now removed) against a plain Booking.com search-results URL, and the real feature has since been built on top of that validation.

**How it actually works, since it's a different mechanism from everything else in this document:** Deep Link Automation is a client-side script (`am.js`, included on every page via `CJ_DEEPLINK_SCRIPT_URL` in `templates/base.html`) that detects ordinary, un-tracked links to known advertiser domains already present on the page and rewrites them for attribution *in the visitor's browser*, at click time. Wanderes never constructs a CJ tracking URL, never generates a `cjevent`, and never touches CJ's Link Search API (`CJAffiliateProvider`, §13.8) for this - the two CJ products are unrelated and use different credentials (this one needs neither `CJ_API_TOKEN` nor `CJ_WEBSITE_ID`).

**What was built:** `integrations/accommodations/` - a new, small `AccommodationSearchLinkProvider` interface (one method: build a search URL), deliberately separate from `HotelProvider` (§13.7) since that interface assumes inventory search Wanderes still doesn't have from anyone. `BookingComSearchLinkProvider` builds a plain `https://www.booking.com/searchresults.en-gb.html?ss=<City>%2C+<Country>` link - destination-only when that's all Wanderes knows (the live chat flow's actual case), with dates/party size/child ages included when a caller genuinely has them. The "Search stays in {destination}" CTA appears on a recommendation's detail card (`ai/views.py`, `ai/templates/ai/chat.html`), paired with a first-party `accommodation_outbound_click` analytics event - fully documented in `documentation/16_ANALYTICS_ARCHITECTURE.md` §16, including the explicit boundary that this first-party event and CJ's own client-side attribution never share data.

**Recommendation independence still holds, unchanged from §13.3/§14** - nothing about this feature (click data, CTR, which destinations get searched most) feeds `recommendations/scoring.py`. See that module's own comment.
