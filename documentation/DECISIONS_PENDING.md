...
## 4. Flight & Hotel Affiliate Provider Selection (blocks Phase 24) — 🔵 RESEARCH DONE, decision pending for flights (KAYAK, not yet accessible); Booking.com property-level data access CLOSED 2026-09-11, see below
...
**Update 2026-09-06**: the user has actually initiated the Booking.com partnership process, via **CJ Affiliate** (a third-party affiliate network) rather than applying to Booking.com directly. Concrete program details now on file: program "Booking.com Spain & Portugal," CJ Advertiser ID `4347393`, category Hotel, currency EUR, base commission currently displayed 4% (1-day reference period), deep linking allowed. The full intended flow, the CJ program's usage restrictions, and - critically - the distinction between CJ Affiliate access (tracking/commission/deep links, now in progress) and Booking.com's separate **Demand API** (programmatic property/price/availability data, **not** automatically granted by CJ approval) are recorded in `10_EXTERNAL_INTEGRATIONS.md` §13.8. **This does not resolve the open decision below** - it's progress on the affiliate/commercial relationship, not on the data-access question. `integrations/hotels/booking_com.py` remains the same deliberate `NotImplementedError` skeleton (§13.7); nothing was implemented by this update, per direct instruction - the next actionable step is confirming Demand API access/credentials once (if) Booking.com grants them.

**Update 2026-09-06, same day - application submitted, and Demand API access explicitly confirmed as NOT granted**: the CJ account is created/configured and a formal application was submitted for the program above - **status: pending, awaiting Booking.com's manual review** (CJ itself confirmed Booking.com approves publishers manually, no instant-approval path). Several creative resources are already visible under the program listing even pre-approval (Accommodations widget `17323139`, Flights widget `17323141`, Car Rentals widget `17323142`, evergreen links) - their visibility is **not** authorization to use them while the application is pending. Separately, and more importantly: **the Booking.com Affiliate Partner Centre was actually tested and returned "You have no access rights to the Affiliate Partner Centre. Contact your administrator to request access rights."** - direct, confirmed evidence (not an assumption) that the CJ account does not grant Affiliate Partner Centre access, and therefore that Demand API access remains unconfirmed. Full detail in `10_EXTERNAL_INTEGRATIONS.md` §13.8's same-day status-update block. **Explicit instruction: the pending application must not block other product development** - the `AccommodationProvider` abstraction stays decoupled from Booking.com specifically for this reason. Still nothing implemented; still no code, settings, or test changes.

**Update 2026-09-06, later same day - Priceline Partner Solutions named as a candidate to evaluate (research/documentation only, no decision, no code)**: during an unrelated UX/product review, the user mentioned Priceline Partner Solutions as a travel-inventory partnership being pursued alongside Booking.com, and specifically flagged its documented accommodation model (`Express.Availability` combined with Hybrid Deep Linking) as a "potentially strong architectural fit" for the same `AccommodationProvider` abstraction already scaffolded for Booking.com (§13.7/§13.8). **This is not a decision** - Priceline was explicitly not researched against the same criteria as the KAYAK/Booking.com/Duffel/Skyscanner table above (access model, traffic minimums, commission structure, geographic coverage), and the user's own instruction alongside this was explicit: do not implement Priceline integration, do not create mock Priceline responses, do not add UI implying live hotel inventory already exists. No `integrations/` skeleton was created for Priceline (unlike KAYAK/Booking.com, which got one only after the user separately asked to "prepare the internal interface now" for each) - it's recorded here purely so a future session doesn't lose track of it as a named candidate. Whether to pursue it, alongside or instead of Booking.com, and on what commercial terms, remains squarely a decision for the human, same as every other provider in this section.

**Update 2026-09-09 - a real CJ personal access token provided, a real CJ Link Search adapter built (not a skeleton), Booking.com Demand API status unchanged.** Full account in `10_EXTERNAL_INTEGRATIONS.md` §13.8 and `DEVELOPMENT_LOG.md` - summary: CJ's own Link Search API (link/keyword/advertiser discovery, real trackable deep links) is genuinely usable now and has a working `integrations/affiliates/CJAffiliateProvider` adapter plus a new `AffiliateLink` model (`integrations/models.py`) to persist real results. This is **not** Booking.com's Demand API (no property/price/availability search) - that access remains unconfirmed, exactly as it was after the 2026-09-06 check. `integrations/hotels/booking_com.py` is untouched. `CJ_WEBSITE_ID` (a second CJ credential, separate from the token) is still unset - nothing can make a real call yet. Nothing wired into recommendations/scoring or any traveler-facing surface.

**Update 2026-09-11 - CLOSED: Booking.com property-level data access confirmed unavailable to Wanderes, via direct correspondence with CJ.** The founder emailed CJ's Booking.com program contact (CJ_Booking, routed to Partner Manager Ophelia Jian) asking specifically whether any property-level data source - a product feed, a Travel & Experience feed, CJ Product Search, a hotel catalogue, or any other approved method - exists for CJ publishers to programmatically discover/identify Booking.com properties, given Demand API access itself was already known to be out of reach (see the 2026-09-06 entries above). CJ's answer, verbatim: **"Unfortunately, no property-level feed is currently available."** Confirmed alongside that Demand API access is restricted to "strategic partners" in a scope Wanderes's publisher category doesn't fall into, and that relationship is managed entirely through CJ, not directly with Booking.com. What CJ does offer - deep links to specific hotel pages, and search widgets for accommodations/flights/car rentals - is exactly what `integrations/affiliates/CJAffiliateProvider` (built 2026-09-09) already targets; nothing about that adapter changes.

**Decision, made directly by the founder based on this answer**: Wanderes will not build personalized, property-level hotel recommendations against Booking.com data - there is no approved way to do it as a CJ publisher in this category, now or without a status change CJ said they'd flag if it happens. The `integrations/hotels/` scaffolding that existed specifically in anticipation of Booking.com API access - `integrations/hotels/booking_com.py`'s skeleton adapter and its registration in `get_hotel_provider()` - has been removed as of this date (see `DEVELOPMENT_LOG.md`'s matching entry). `integrations/hotels/base.py`'s provider-agnostic `HotelProvider`/`HotelOption` interface was kept, since it isn't Booking.com-specific and Priceline Partner Solutions (named 2026-09-06 below) remains a genuinely separate, unresearched candidate for accommodations - this closes out Booking.com specifically, not "hotels" as a category. `HOTEL_PROVIDER` stays unset in `.env.example`/`render.yaml`, same as before, since no adapter is registered anymore.
...
- ~~Which hotel provider(s) to pursue~~ - **Booking.com closed 2026-09-11** (see the update above: no property-level data feed available to Wanderes's publisher category). Priceline Partner Solutions (2026-09-06 below) remains the only other named-but-unresearched accommodation candidate.
...
The `FlightProvider`/`HotelProvider` internal interfaces described in `10_EXTERNAL_INTEGRATIONS.md` §3 are scaffolded (§13.6/§13.7 - `integrations/flights/`, `integrations/hotels/`). `integrations/flights/kayak.py` stays a deliberate `KayakFlightProvider` skeleton raising `NotImplementedError` rather than guessing at an undocumented shape. **`integrations/hotels/booking_com.py` no longer exists** (removed 2026-09-11, see the update above) - the `HotelProvider`/`HotelOption` interface itself is still there, ready for whichever accommodation provider is decided next. Once real API access/documentation exists for a flight provider, or a new accommodation provider is chosen, filling in that adapter is the only code change needed - per Phase 24's Claude Code responsibilities in `15_IMPLEMENTATION_GUIDE.md`. Account creation with any provider, and any commercial/API agreement, remains something only you can do - Claude Code cannot create provider accounts or assume affiliate eligibility on your behalf.
...
## 5. Google OAuth Login — ✅ RESOLVED 2026-09-11 (real Google Cloud credentials set on Render)
...
**Update 2026-09-11: resolved.** `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` are now set on Render's `wanderes-web` service - the "Continue with Google" button should render live on `/users/login/` and `/users/register/`. No code change needed; this entry stays for reference on how the credentials were obtained.
...
**Update 2026-09-11 - provider decided: Purelymail.** `render.yaml`/`.env.example` now hardcode `EMAIL_HOST=smtp.purelymail.com`, `EMAIL_PORT=587`, `EMAIL_USE_TLS=True` (Purelymail's real, documented settings - not user-specific, so committing them isn't a secrets leak). This isn't a decision Claude Code made unilaterally - the user named the provider directly; only the already-public technical settings were filled in, verified live against [Purelymail's own docs](https://purelymail.com/docs/setup/technical) before being written into the config. `EMAIL_HOST_USER`/`EMAIL_HOST_PASSWORD` remain real account credentials and stay `sync: false`.

### What you need to do (Purelymail account + DNS, your own domain access)

1. Create/confirm a Purelymail account and add `wanderes.com` as a custom domain - it will show an ownership-proof TXT record, an MX record, and an SPF record to add wherever `wanderes.com`'s DNS is managed; DKIM records follow once ownership verifies. Full steps: [purelymail.com/docs/setup/technical](https://purelymail.com/docs/setup/technical).
2. Create a mailbox to send from (e.g. `noreply@wanderes.com`, matching `DEFAULT_FROM_EMAIL`).
3. On Render: `wanderes-web` service → Environment → set `EMAIL_HOST_USER` (the full Purelymail address) and `EMAIL_HOST_PASSWORD` (its account password, or an app password if 2FA is on) - still `sync: false`, entered manually, never in source control. `EMAIL_HOST`/`EMAIL_PORT`/`EMAIL_USE_TLS`/`DEFAULT_FROM_EMAIL` no longer need touching - they're already correct for Purelymail.
4. No redeploy needed for the env var change itself - the "Forgot your password?" link appears automatically once both are set.

### What Claude Code cannot do

Create the Purelymail account, agree to its terms, or edit `wanderes.com`'s DNS records - the same boundary already documented for Google OAuth and KAYAK above.

**Reference:** `users/urls.py` (the four password-reset views), `users/templates/users/password_reset_*.html/.txt`, `config/settings/base.py`'s `EMAIL_*` settings, `users/tests/test_password_reset.py`, `render.yaml`/`.env.example`.
