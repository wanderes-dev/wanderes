...
**2026-09-04, direct user request (automatic language detection spec)**: priority tier 4 of the requested detection order - "approximate IP geolocation as a fallback/context signal... never treat it as definitive proof of the user's preferred language." Everything else in that request is built (see the same-day `DEVELOPMENT_LOG.md` entries - **briefly reverted after a production incident caused by an unrelated migration-timing issue, not a flaw in this design, then re-applied the same day once the user confirmed the migration; see `PROJECT_STATE.md` item 3 - verify live before treating this as fully settled**) - saved preference priority (account-level for authenticated visitors, cookie for anonymous), full browser Accept-Language detection, and a dismissible "prefer Wanderes in X?" suggestion banner driven entirely by the browser's own signal, no IP involved.
