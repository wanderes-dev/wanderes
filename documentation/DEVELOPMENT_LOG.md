...
## 2026-08-30 — Render deploy succeeded; root-path 404 fixed

Before this could go live, one gap in the Phase 18 deploy prep needed fixing: `render.yaml`'s `DJANGO_SECRET_KEY` was split across `generateValue: true` (web) and `sync: false` (worker) - the first fills in silently during Blueprint creation with nothing shown to the user, so there was no way to know what value to paste into the worker's prompt, which appears in the same form before the web service (and its silently-generated value) exists. Caught this by reasoning through the actual Blueprint creation flow before the user hit it, not after. **Fixed**: both services now use `sync: false`, and the README instructs generating one random value upfront (`python -c "import secrets; print(secrets.token_urlsafe(50))"`) to paste into both prompts.

**The user then created the Render account and deployed for real** - the first actual validation of `render.yaml` against a live Render account, since this repo never had one during prep. **It worked on the first attempt** - no Blueprint syntax errors, confirming the earlier decision to research Render's current documented spec via live `WebFetch` calls (rather than relying on possibly-stale field names) paid off. The app is live at https://wanderes.onrender.com/.

**One real issue surfaced immediately by an actual visitor hitting the bare domain**: `https://wanderes.onrender.com/` returned Django's default 404 page. Root cause, confirmed by reading `config/urls.py`: the app never had a URL pattern for the empty path `""` - only `/chat/`, `/health/`, `/users/...`, `/trips/...`. This was true the entire time the app only ran on `localhost` (nobody visits a bare `http://localhost:8000/` by habit the way they'd type a real domain), so a real deploy was exactly what was needed to surface it. **Fixed**: added `path("", RedirectView.as_view(pattern_name="ai:chat", permanent=False), name="root")` to `config/urls.py` - a temporary (302, not 301) redirect to `/chat/`, since a real landing page might replace this later and a 301 would get cached by browsers/crawlers in a way that's annoying to undo. 1 new test (`core/tests/test_root_redirect.py`), 166/166 total passing, `ruff check .` clean. Verified against the real local server before pushing (`curl -I http://localhost:8000/` → `302` → `/chat/`); Render auto-redeploys on push to the connected branch, so this fix reaches production without any manual redeploy step.
