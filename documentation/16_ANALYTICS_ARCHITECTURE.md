...
## 16. Update 2026-09-23 — Accommodation search outbound clicks + CJ affiliate attribution

Adds one new event, one new `fact_recommendations` column, and a small query layer — no new architecture, everything above still applies unchanged.

**New event: `accommodation_outbound_click`.** Fires when a traveler clicks the "Search stays in {destination}" link on a recommendation's detail card (only the single-destination detail path gets this CTA, not the multi-destination browse cards — see `ai/views.py::_recommendation_card_data`). Actor rules follow every other product event (§3, §8) — user or anonymized IP, never both. `metadata` carries `destination_slug`, `provider` (`"booking_com"`), the destination's own `trip_type`/`cost_of_living` (always real, from `travel.Destination`), and, only for an authenticated visitor with a saved `TravelerProfile`, `traveler_preferred_trip_types`/`traveler_preferred_cost_of_living` — a snapshot of real, already-collected preference fields at click time, never an invented taxonomy (no "pace"/"vibe"/"interests" — those don't exist on `TravelerProfile`).

**CJ attribution is a strictly separate mechanism, and never touches this event.** `templates/base.html` conditionally includes CJ's Deep Link Automation script (`CJ_DEEPLINK_SCRIPT_URL`, `core.context_processors.site_meta`'s `cj_deeplink_script_url`) — a client-side script that detects the plain Booking.com search link on the page and rewrites it for attribution *at the browser*, independently of anything Wanderes's own JS does. The `accommodation_outbound_click` click handler in `ai/templates/ai/chat.html` never calls `preventDefault()`/`stopPropagation()` specifically so it can't interfere with that rewriting. Wanderes's own event never stores a CJ tracking id, `cjevent`, cookie, or redirect URL — `ai/views.py::accommodation_click` only ever reads `destination_slug` from the POST body and resolves everything else itself. This is the "two separate concerns, never merged" boundary the original spec asked for.

**Outbound link generation: `integrations/accommodations/`** (new module, not an extension of `integrations/hotels/`). `HotelProvider` (in `hotels/`) models real inventory search/details/affiliate-link-from-a-result — the wrong shape here, since Wanderes has no accommodation inventory access from anyone (Booking.com confirmed this 2026-09-11, `DECISIONS_PENDING.md` §4). `AccommodationSearchLinkProvider` is a one-method ABC (`build_search_url`) that only ever builds a plain, un-tracked Booking.com search-results URL — same ABC+factory+settings-key pattern (`ACCOMMODATION_SEARCH_PROVIDER`) as every other provider in this codebase. Every parameter past destination/country is optional and omitted rather than guessed when unknown — the live chat flow never knows real check-in/check-out dates or party size, so it only ever passes a destination name and country, and the resulting URL is a valid destination-only search. Child ages, when both `children` and a matching-length `child_ages` list are provided, are encoded as Booking's real documented format (`age=N` once per child, in order) — verified against Booking's own docs before implementing, not guessed.

**`fact_recommendations` gained `was_accommodation_clicked`** (§5.5), correlated via the same `conversation_key`/`episode_number`/`destination_slug` join as `was_selected`/`was_saved`, with one deliberate difference: it's attributed to the **nearest preceding recommendation only** (an `AND NOT EXISTS` guard rejects a row if a more recent recommendation of the same destination exists between it and the click). Verified empirically (not just reasoned about) that without this guard, a destination recommended twice in one episode and clicked once had *both* recommendation rows marked as clicked — silently double-counting one click in any `COUNT(*)`-based CTR aggregation. `was_selected`/`was_saved` keep their original any-later-event correlation, unchanged - see `analytics/warehouse/sql/fact_recommendations.sql`'s own header comment for why the three aren't symmetric. This means **no separate "impression" event was needed** — a `fact_recommendations` row already *is* the eligible-exposure record (the destination was actually recommended in that episode), so `COUNT(*)` per destination is the impression denominator and `SUM(was_accommodation_clicked::int)` the click numerator; CTR is one `GROUP BY` away, no new table.

**Query layer: `analytics/queries.py`** (new, small, DB-aggregated functions, no dashboard framework) — `top_clicked_destinations`, `accommodation_click_through_rate_by_destination`, `top_destinations_by_traveler_trip_type`. Not wired into `/analytics/dashboard/` this pass (out of scope for this change — the dashboard itself wasn't touched); these are library functions plus tests, callable from a shell or a future dashboard addition.

**Ranking independence, restated as a hard rule, not just a convention:** `recommendations/scoring.py` carries an explicit comment (near its module-level constants) stating this data must never influence a score. Nothing in `analytics/queries.py` is imported by `recommendations/` or `ai/orchestration.py`.

**Removed:** the temporary `/test-cj-deeplink/` validation page (`core/views.py`, `core/urls.py`, its template, its dedicated test file, and the `robots.txt` disallow entry it added) — it had already served its purpose (validating the Deep Link Automation script against a real Booking.com URL in production) before this pass began.

**Files changed/added this update:** `integrations/accommodations/{base,booking_com,__init__}.py` (new); `integrations/tests/test_accommodations.py` (new); `config/settings/base.py` (`ACCOMMODATION_SEARCH_PROVIDER`, `CJ_DEEPLINK_SCRIPT_URL`); `.env.example`; `core/context_processors.py` (`cj_deeplink_script_url`); `templates/base.html` (conditional CJ script tag); `analytics/models.py` (`accommodation_outbound_click` choice); `analytics/migrations/0006_*.py`, `0007_*.py`; `analytics/warehouse/sql/fact_recommendations.sql`, `analytics/warehouse/models.py` (`was_accommodation_clicked`); `analytics/warehouse/tests/test_views.py`; `analytics/queries.py` (new) + `analytics/tests/test_queries.py` (new); `recommendations/scoring.py` (ranking-independence comment); `ai/views.py` (`_recommendation_card_data`'s `accommodation_search_url`, new `accommodation_click` view, `searchStaysIn` i18n string); `ai/urls.py`; `ai/tests/test_views.py`; `ai/templates/ai/chat.html` (CTA link + `fetch(..., keepalive: true)` click tracking); `core/views.py`, `core/urls.py`, `core/tests/test_seo.py` (removal of the temporary page); `documentation/10_EXTERNAL_INTEGRATIONS.md` (§13.9).

## 17. Update 2026-09-24 — First-party acquisition attribution

Answers "which acquisition channel (Instagram/TikTok organic or paid, Google/search organic, direct/referral) actually produces meaningful travel intent" - end to end from `?utm_source=...` on a landing request through to an accommodation-search click, entirely first-party, reusing every piece of infrastructure above rather than building a second analytics system.

**New module: `analytics/acquisition.py`.** Not a middleware - `capture_acquisition(request)` is called explicitly from the app's two real landing surfaces (`core.views.landing`, `ai.views.chat_page`), so it never runs for API calls, health checks, or static assets, matching §6's "no event on every ordinary page request." Two touches live in the visitor's existing Django session (the same DB-backed session `ai.memory` already keys anonymous conversations by - no new identity mechanism):

- **`first_touch`** - captured once, the first time a session is ever seen, and never overwritten afterward, regardless of how many times the visitor navigates internally without UTMs.
- **`latest_touch`** - starts equal to `first_touch`, and only updates when a request arrives carrying a genuinely *different* set of UTM parameters than what's already stored as the latest touch. This dual check (UTMs present AND different from the current latest touch) is what makes a page refresh of the exact same campaign URL a correct no-op instead of a duplicate event - confirmed live, not just in tests: refreshing an identical `?utm_source=tiktok&...` URL never creates a second `acquisition_captured` row.

**Capture logic, in order:** explicit `utm_source`/`utm_medium`/`utm_campaign`/`utm_content`/`utm_term` (any subset - none required, none invented) always win when present. Only when *no* UTM parameter is present at all does a small, deterministic `Referer`-based classifier run: Google/Bing/DuckDuckGo/Yahoo → `organic_search`; Instagram/TikTok → `organic_social`; any other site → `referral` (with that site's bare hostname as `source` - never the full URL, which could carry a search query or other detail not worth retaining); nothing at all → `direct`. Every UTM value is sanitized before it's ever stored: stripped to an allowlist of letters/digits/space/`_-.+ ` (removing HTML/script metacharacters like `<`, `>`, `"`, `'`, `;`, `\`, `/` by construction, not by pattern-matching specific attacks) and capped at 100 characters.

**Event: `acquisition_captured`.** Fires once per genuinely new touch (first or latest, never a refresh/internal-nav no-op). `metadata` = `{touch_type, source, medium, campaign, content, term}` - a flat shape (not nested), since this event only ever describes its own single touch, unlike the snapshot below.

**Historical correctness - the compact snapshot.** A visitor's session state is mutable (their `latest_touch` can change on a return visit weeks later); an already-recorded event's meaning must never change retroactively because of that. So `travel_question_submitted`, `recommendation_generated`, and `accommodation_outbound_click` each get a **frozen** `metadata.acquisition = {first_touch: {...}, latest_touch: {...}}` snapshot attached at the moment they're recorded (via `analytics.acquisition.get_acquisition_snapshot(request)`), not a live pointer into the session. Omitted entirely (not padded with nulls) when the session never went through `capture_acquisition()` at all. `destination_selected` deliberately does **not** get this - out of scope, matching the spec's own named list of exactly these three downstream stages.

**Warehouse: `fact_acquisition_funnel`** (new view + migration). Grain: `(conversation_key, episode_number, touch_type)` - almost every episode has exactly one row (`touch_type='first'`); a second `'latest'` row only appears if the same visitor clicked a genuinely new campaign link within the same 30-minute episode window, a real but rare case. Sources from `acquisition_captured` events, correlating each against later `travel_question_submitted`/`recommendation_generated`/`accommodation_outbound_click` events in the *same* episode via the same correlated-`EXISTS` pattern `fact_recommendations` already established - `reached_planning`, `reached_recommendation`, `reached_accommodation_click`. No new "impression" event needed, matching the same reasoning that shipped `was_accommodation_clicked` without a separate impression table.

**Query layer: `analytics/queries.py`.**
- `acquisition_funnel(start_date, end_date, touch_type="first")` - sessions/planning_starts/recommendations/accommodation_clicks grouped by source/medium/campaign/content, raw counts (not pre-divided rates - the caller picks whatever denominator the actual question calls for). `touch_type="latest"` answers the complementary "which channel most recently brought someone back" question.
- `acquisition_by_traveler_preference(start_date, end_date, trip_type=None, cost_of_living=None)` - accommodation-click acquisition breakdown segmented by a real, already-captured `TravelerProfile` preference dimension (never a live join, never an inferred demographic) - answers "which channel attracts budget travelers" / "which content attracts beach travelers" using exactly the `traveler_preferred_trip_types`/`traveler_preferred_cost_of_living` snapshot fields `accommodation_outbound_click` already carried before this pass.

Deliberately called **`accommodation_clicks`**, never "conversions" or "bookings," everywhere in this layer - an outbound click to Booking.com is the only thing Wanderes actually observes.

**Dashboard: query layer only, no UI this pass.** Adding an acquisition section to `/analytics/dashboard/` would have materially expanded this change's scope (a new template section, new `_query()` calls, date-range UI) on top of everything else already shipped here - deferred as a separate, smaller follow-up task, per the spec's own explicit permission to do exactly that when the alternative is scope creep.

**Ranking independence, extended explicitly.** `recommendations/scoring.py`'s existing boundary comment now also names acquisition data - source, campaign, ad spend, channel-level CTR - as something that must never reach scoring. A TikTok visitor and a direct visitor with identical stated preferences get identical rankings; nothing in `stream_travel_recommendation()`'s signature even accepts an acquisition parameter, so this is a structural guarantee, not just a documented intention - regression-tested by asserting the exact call made to it is identical regardless of which channel the calling session came from.

**Privacy.** No fingerprinting, no cross-site tracking, no new IP storage beyond the same anonymized-IP mechanism every other event already uses, no CJ identifiers, no Booking cookies, no ad-platform identifiers (no GA/GTM/Meta Pixel/TikTok Pixel/anything third-party was installed - first-party only, per direct instruction). Referrer classification never stores a full URL, only a coarse category and (for an unrecognized site only) its bare hostname. Works identically for anonymous and authenticated visitors - account creation was never required for attribution to function.

**UTM naming convention** (documented here so future campaigns stay comparable, not enforced as a closed enum since campaign experimentation is expected to keep evolving):
- **Sources:** `instagram`, `tiktok`, `google`, `direct`, `referral` (or a specific referring hostname for an unrecognized site)
- **Mediums:** `organic_social`, `paid_social`, `organic_search`, `referral`, `direct`
- **Campaign:** names the experiment/theme (e.g. `destination_discovery`, `warm_november`) - not a date, not a one-off human-readable variation, so the same campaign name stays comparable across its own run
- **Content:** identifies the specific creative/video/ad variant (e.g. `budget_1500`, `creative_01`) - this is the field that lets `acquisition_by_traveler_preference`/`acquisition_funnel` answer "which specific TikTok video actually produced useful travelers," not just "TikTok vs. Instagram"

**Verification:** 592/592 tests passing (~45 new across `analytics/tests/test_acquisition.py` [new], `analytics/tests/test_queries.py`, `analytics/warehouse/tests/test_views.py`, `ai/tests/test_views.py`), `ruff check .` clean, 2 new migrations. Live-verified end to end in an isolated Docker stack against a real chat message and real destination catalog: landed with `?utm_source=tiktok&utm_medium=organic_social&utm_campaign=destination_discovery&utm_content=budget_1500`, sent a real chat message, clicked an accommodation link - `acquisition_funnel()` correctly returned `sessions=1, planning_starts=1, recommendations=1, accommodation_clicks=1` for that exact source/campaign/content combination. Separately confirmed a refresh of the identical UTM URL creates no duplicate `acquisition_captured` event.

**Files changed/added this update:** `analytics/acquisition.py` (new); `analytics/models.py` (`acquisition_captured` choice); `analytics/migrations/0008_*.py`, `0009_*.py`; `analytics/warehouse/sql/fact_acquisition_funnel.sql` (new), `analytics/warehouse/models.py` (`FactAcquisitionFunnel`), `analytics/warehouse/tests/test_views.py`; `analytics/queries.py`, `analytics/tests/test_queries.py`; `analytics/tests/test_acquisition.py` (new); `recommendations/scoring.py` (ranking-independence extension); `core/views.py` (`landing` calls `capture_acquisition`); `ai/views.py` (`chat_page` calls `capture_acquisition`; `recommendations_stream`/`accommodation_click` attach the snapshot); `ai/tests/test_views.py`.
