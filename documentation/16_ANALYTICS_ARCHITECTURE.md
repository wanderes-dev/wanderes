...
**`fact_recommendations` gained `was_accommodation_clicked`** (§5.5), same correlated-`EXISTS` pattern as `was_selected`/`was_saved` — the click event's own `conversation_key`/`episode_number`/`destination_slug` join it against the recommendation that produced it. This means **no separate "impression" event was needed** — a `fact_recommendations` row already *is* the eligible-exposure record (the destination was actually recommended in that episode), so `COUNT(*)` per destination is the impression denominator and `SUM(was_accommodation_clicked::int)` the click numerator; CTR is one `GROUP BY` away, no new table.
