...
**Current phase:** Phases 0-9 complete. `OPENAI_API_KEY` is now configured in `.env` (real key, never committed) — the AI adapter and orchestrator have both been validated live, not just against mocks. See the Phase 9 entry above for a real bug this surfaced and fixed (qualitative constraints like "warm" weren't being translated into actual filter thresholds). Next: Phase 10 (chat interface) — the first thing that will let a person actually talk to Lunna through a browser, with real conversation history and streaming.
