...
- [~] **Phase 11 — First End-to-End MVP (Joint Review)** ⚠️ In progress 2026-08-29. Ran all 7 real scenarios named in `15_IMPLEMENTATION_GUIDE.md` (warm, budget, romantic, beach holiday, city break, family travel, strong exclusions) live against the real OpenAI API + real climate data. Findings:
  ...
  - ❌ **Still open, NOT fixed, and NOT asked for yet:** "romantic destination" and "family travel" scenarios still fall entirely on the AI's own reasoning with zero deterministic scoring/filtering — there is no data model concept of "romantic" or "family-friendly" at all (unlike trip_type, this needs a genuine product decision about whether/how to represent it, not just wiring — deliberately not decided unilaterally). Also still open: in a couple of replies the AI added supplementary facts not present in our dataset (e.g. "Lisbon has nearby beaches," "Kyoto is known for safety") — a grounding concern not yet addressed.
  - **Human Decision still pending**: whether the experience (with these two remaining gaps) is good enough to continue expanding, per this phase's own framing. Not yet closed out.
