...
- **The OpenAI adapter exists** (`ai.provider.OpenAIProvider`, same pattern as the climate adapter: ABC interface, `get_ai_provider()` factory, `AI_PROVIDER`/`AI_MODEL` settings). **`OPENAI_API_KEY` in `.env` is blank** — the adapter raises a friendly `ImproperlyConfigured` until the user adds a real key; don't "fix" this by hardcoding a key or working around the check. `ai/prompts.py` has the `SYSTEM_PROMPT` (assistant name "Lunna") — not auto-injected by the adapter, meant for the orchestration layer (Phase 9, not yet built) to prepend.
