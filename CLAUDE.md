...
- **External provider adapters live behind an internal interface, per app/domain** (`10_EXTERNAL_INTEGRATIONS.md` §3) — e.g. `integrations.climate.ClimateProvider` (ABC) + `OpenMeteoClimateProvider` (adapter), selected via a `get_*_provider()` factory reading a settings key (`CLIMATE_PROVIDER`), so swapping providers is a settings change. The `integrations` app holds no models — it's pure Python + one `AppConfig`. Follow this pattern for the OpenAI adapter when it's built.
- Phase 6 (traveler profile edit/retrieve) was **deliberately skipped ahead of** in favor of Phase 7 (travel data integration), per explicit user instruction on 2026-08-29 — it's still outstanding, not forgotten. Don't assume phases were done in strict numeric order; always check `PROJECT_STATE.md`'s checklist.
