...
- **`trips.TravelHistoryEntry`** (added 2026-08-29, Phase 12) is a separate, simpler concept from `Trip` — "visited X, roughly year Y," CRUD at `/trips/history/`, no dates/items. Both `Trip.status="completed"` and `TravelHistoryEntry` feed the same repetition-penalty scoring in `recommendations.scoring`. `trips` app's first views/urls/templates live here.
