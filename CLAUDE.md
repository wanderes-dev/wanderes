...
- **The OpenAI adapter exists** (`ai.provider.OpenAIProvider`, same pattern as the climate adapter: ABC interface, `get_ai_provider()` factory, `AI_PROVIDER`/`AI_MODEL` settings) and supports `generate_reply`, `generate_structured_reply` (structured JSON output), and `stream_reply` (incremental chunks). **`OPENAI_API_KEY` is now configured in `.env` with a real key** (added 2026-08-29, never committed) — the AI pipeline is validated live, not just against mocks. `ai/prompts.py` has the `SYSTEM_PROMPT` (assistant name "Lunna").
